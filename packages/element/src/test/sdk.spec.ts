...
describe('Element', () => {
  let element: Element;
  let signingKeyPair: [PublicKeyJwk, PrivateKeyJwk];
  ...
  it('should generate key material', async () => {
    ...
    signingKeyPair = await sdk.Keys.generateKeyPairFromMnemonic(mnemonic, 0);
    expect(signingKeyPair[0]).toBeDefined();
    expect(signingKeyPair[1]).toBeDefined();
