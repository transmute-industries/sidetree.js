...
describe('AnchorFile', () => {
  describe('parse()', () => {
    it('should parse an anchor file model correctly.', async () => {
      ...
      const recoverOperationData = await OperationGenerator.generateRecoverOperation(
        {
          ...
          recoveryPrivateKey: anyRecoveryPrivateKey,
        }
      );
      ...
      expect(parsedAnchorFile.createOperations[0].encodedSuffixData).toEqual(
        createOperation.encodedSuffixData
      );
      expect(parsedAnchorFile.recoverOperations.length).toEqual(1);
      expect(
        parsedAnchorFile.recoverOperations[0].signedDataJws.toCompactJws()
      ).toEqual(recoverOperation.signedDataJws.toCompactJws());
      expect(parsedAnchorFile.deactivateOperations.length).toEqual(1);
      expect(
        parsedAnchorFile.deactivateOperations[0].signedDataJws.toCompactJws()
      ).toEqual(deactivateOperation.signedDataJws.toCompactJws());
    ...
    it('should throw if has an unknown property.', async () => {
      ...
      await JasmineSidetreeErrorValidator.expectSidetreeErrorToBeThrownAsync(
        () => AnchorFile.parse(anchorFileCompressed),
        ErrorCode.AnchorFileHasUnknownProperty
      );
    });
    ...
    it('should throw if missing map file hash.', async () => {
      ...
      await JasmineSidetreeErrorValidator.expectSidetreeErrorToBeThrownAsync(
        () => AnchorFile.parse(anchorFileCompressed),
        ErrorCode.AnchorFileMapFileHashMissing
      );
    });
    ...
    it('should throw if missing operations property.', async () => {
      ...
      await JasmineSidetreeErrorValidator.expectSidetreeErrorToBeThrownAsync(
        () => AnchorFile.parse(anchorFileCompressed),
        ErrorCode.AnchorFileMissingOperationsProperty
      );
    });
    ...
    it('should throw if map file hash is not string.', async () => {
      ...
      await JasmineSidetreeErrorValidator.expectSidetreeErrorToBeThrownAsync(
        () => AnchorFile.parse(anchorFileCompressed),
        ErrorCode.AnchorFileMapFileHashNotString
      );
    });
    ...
    it('should throw if `create` property is not an array.', async () => {
      ...
      await JasmineSidetreeErrorValidator.expectSidetreeErrorToBeThrownAsync(
        () => AnchorFile.parse(anchorFileCompressed),
        ErrorCode.AnchorFileCreatePropertyNotArray
      );
    });
    ...
    it('should throw if `recover` property is not an array.', async () => {
      ...
      await JasmineSidetreeErrorValidator.expectSidetreeErrorToBeThrownAsync(
        () => AnchorFile.parse(anchorFileCompressed),
        ErrorCode.AnchorFileRecoverPropertyNotArray
      );
    });
    ...
    it('should throw if `deactivate` property is not an array.', async () => {
      ...
      await JasmineSidetreeErrorValidator.expectSidetreeErrorToBeThrownAsync(
        () => AnchorFile.parse(anchorFileCompressed),
        ErrorCode.AnchorFileDeactivatePropertyNotArray
      );
    });
  ...
  describe('createModel()', () => {
    it('should created an anchor file model correctly.', async () => {
      ...
      expect(anchoreFileModel.operations.create![0].suffix_data).toEqual(
        createOperation.encodedSuffixData
      );

      // Verify recover operation.
      const recoveryOperationInAnchorFile = anchoreFileModel.operations
        .recover![0];
      expect(recoveryOperationInAnchorFile.did_suffix).toEqual(
        recoverOperation.didUniqueSuffix
      );
      expect(recoveryOperationInAnchorFile.signed_data).toEqual(
        recoverOperation.signedDataJws.toCompactJws()
      );

      // Verify deactivate operation.
      const deactivateOperationInAnchorFile = anchoreFileModel.operations
        .deactivate![0];
      expect(deactivateOperationInAnchorFile.did_suffix).toEqual(
        deactivateOperation.didUniqueSuffix
      );
      expect(deactivateOperationInAnchorFile.signed_data).toEqual(
        deactivateOperation.signedDataJws.toCompactJws()
      );
    });
  });
