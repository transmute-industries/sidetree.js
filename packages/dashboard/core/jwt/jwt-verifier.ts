// I would use https://github.com/auth0/node-oauth2-jwt-bearer/tree/main/packages/express-oauth2-jwt-bearer
// But it is using util.Decoder that does not work and errors when tried to use.
// https://github.com/auth0/node-oauth2-jwt-bearer/blob/d7259e5464494eddd0ccda0da2463af9ac753a60/packages/access-token-jwt/src/fetch.ts#L6
// ReferenceError: TextEncoder is not defined
// Based off of https://github.com/auth0/node-oauth2-jwt-bearer/tree/main/packages/access-token-jwt
...
const jwtVerifier = ({
  ...
  const JWKS = async (...args: Parameters<GetKeyFn>) => {
    ...
    if (!origJWKS) {
      ...
      origJWKS = createRemoteJWKSet(url, {
        agent,
        cooldownDuration,
        timeoutDuration,
      });
  ...
  return async (jwt: string) => {
    try {
      ...
      throw new InvalidTokenError((e as any).message);
    }
  };
};
